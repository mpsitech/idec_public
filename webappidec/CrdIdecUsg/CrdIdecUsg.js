/**
  * \file CrdIdecUsg.js
  * web client functionality for card CrdIdecUsg
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppIdecUsg", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppIdecUsg", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppIdecUsg", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppIdecUsg", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppIdecUsg", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlIdecUsgHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlIdecUsgList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlIdecUsgRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppIdecUsg", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppIdecUsg", "shortMenu", short);
	setSi(srcdoc, "StatAppIdecUsg", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppIdecUsg", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppIdecUsg", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagIdecUsg", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagIdecUsg", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdUsg"), retrieveCi(srcdoc, "ContInfIdecUsg", "MtxCrdUsg"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfIdecUsg", "MtxCrdUsg") + " - ICARUSDetectorControl 0.2.1";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecUsgDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecUsgDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfIdecUsg", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecUsgData", "ContInfIdecUsg", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecUsgData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngIdecUsgData", "StatAppIdecUsg", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngIdecUsgData", "StatShrIdecUsg", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecUsgData", "TagIdecUsg", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecUsgData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrIdecUsg", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrIdecUsg", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrIdecUsg", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecUsgData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxIdecVReqitmode = retrieveSi(srcdoc, "StatAppIdecUsg", "srefIxIdecVReqitmode");

	if (srefIxIdecVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxIdecVReqitmode = "notify";
		setSi(srcdoc, "StatAppIdecUsg", "srefIxIdecVReqitmode", srefIxIdecVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxIdecVReqitmode = retrieveSi(srcdoc, "StatAppIdecUsg", "srefIxIdecVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppIdecUsg", "latency"));
