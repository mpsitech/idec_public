/**
  * \file PnlIdecUsgMNUser.js
  * web client functionality for panel PnlIdecUsgMNUser
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppIdecUsgMNUser", "srefIxIdecVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppIdecUsgMNUser", "srefIxIdecVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNUser_side").src = "./PnlIdecUsgMNUser_aside.html";
	doc.getElementById("MNUser_cont").src = "./PnlIdecUsgMNUser_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppIdecUsgMNUser", "srefIxIdecVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppIdecUsgMNUser", "srefIxIdecVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNUser_side").src = "./PnlIdecUsgMNUser_bside.html";
	doc.getElementById("MNUser_cont").src = "./PnlIdecUsgMNUser_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecUsgMNUser", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagIdecUsgMNUser", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxIdecVExpstate = retrieveSi(srcdoc, "StatAppIdecUsgMNUser", "srefIxIdecVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecUsgMNUser", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryIdecUsgMNUser", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsgMNUser", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsgMNUser", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsgMNUser", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsgMNUser", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsgMNUser", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsgMNUser", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsgMNUser", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsgMNUser", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecUsgMNUser", "TcoMrefWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecUsgMNUser", "TcoUlvWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagIdecUsgMNUser", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryIdecUsgMNUser", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListIdecQUsgMNUser
		rownode = getRownode(srcdoc, "ListIdecQUsgMNUser", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagIdecUsgMNUser", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagIdecUsgMNUser", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfIdecUsgMNUser", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsgMNUser", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsgMNUser", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsgMNUser", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryIdecUsgMNUser", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryIdecUsgMNUser", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecUsgMNUserData", scrJref, "StgIacQryIdecUsgMNUser");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppIdecUsgMNUser", "srefIxIdecVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsgMNUser", "firstcol"));

	setSi(srcdoc, "StatAppQryIdecUsgMNUser", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppIdecUsgMNUser", "srefIxIdecVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsgMNUser", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecUsgMNUserDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecUsgMNUserData", "ContInfIdecUsgMNUser", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecUsgMNUserData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngIdecUsgMNUserData", "StatAppIdecUsgMNUser", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngIdecUsgMNUserData", "StatShrIdecUsgMNUser", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecUsgMNUserData", "StgIacIdecUsgMNUser", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngIdecUsgMNUserData", "TagIdecUsgMNUser", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngIdecUsgMNUserData", "ListIdecQUsgMNUser", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngIdecUsgMNUserData", "StatAppQryIdecUsgMNUser", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngIdecUsgMNUserData", "StatShrQryIdecUsgMNUser", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngIdecUsgMNUserData", "StgIacQryIdecUsgMNUser", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngIdecUsgMNUserData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecUsgMNUserData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecUsgMNUserData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
