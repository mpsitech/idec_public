/**
  * \file PnlIdecUsgList.js
  * web client functionality for panel PnlIdecUsgList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlIdecUsgList_aside.html";
	doc.getElementById("List_cont").src = "./PnlIdecUsgList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlIdecUsgList_bside.html";
	doc.getElementById("List_cont").src = "./PnlIdecUsgList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecUsgList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagIdecUsgList", "Trs"));
...
function init() {
	var srefIxIdecVExpstate = retrieveSi(srcdoc, "StatShrIdecUsgList", "srefIxIdecVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecUsgList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryIdecUsgList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsgList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsgList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsgList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsgList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsgList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsgList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsgList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsgList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecUsgList", "TcoSrfWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacIdecUsgList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagIdecUsgList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryIdecUsgList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListIdecQUsgList
		rownode = getRownode(srcdoc, "ListIdecQUsgList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagIdecUsgList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagIdecUsgList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfIdecUsgList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsgList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsgList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsgList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryIdecUsgList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryIdecUsgList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecUsgListData", scrJref, "StgIacQryIdecUsgList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsgList", "firstcol"));

	setSi(srcdoc, "StatAppQryIdecUsgList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsgList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecUsgListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacIdecUsgList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacIdecUsgList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecUsgListData", scrJref, "ContIacIdecUsgList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecUsgListData", "ContIacIdecUsgList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngIdecUsgListData", "ContInfIdecUsgList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecUsgListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngIdecUsgListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngIdecUsgListData", "StatShrIdecUsgList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecUsgListData", "StgIacIdecUsgList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngIdecUsgListData", "TagIdecUsgList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngIdecUsgListData", "ListIdecQUsgList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngIdecUsgListData", "StatAppQryIdecUsgList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngIdecUsgListData", "StatShrQryIdecUsgList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngIdecUsgListData", "StgIacQryIdecUsgList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngIdecUsgListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecUsgListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecUsgListData") {
				mask = mergeDpchEngData(dom);
