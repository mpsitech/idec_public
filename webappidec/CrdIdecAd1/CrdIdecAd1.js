/**
  * \file CrdIdecAd1.js
  * web client functionality for card CrdIdecAd1
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppIdecAd1", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppIdecAd1", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneControl = (retrieveSi(srcdoc, "StatAppIdecAd1", "initdoneControl") == "true");
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppIdecAd1", "initdoneHeadbar") == "true");

	if (!initdoneControl) {
		doc.getElementById("Control").src = "./PnlIdecAd1Control.html?scrJref=" + scrJrefControl;
	} else if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlIdecAd1Headbar.html?scrJref=" + scrJrefHeadbar;
	} else {
		doc.initdone = true;
...
function getHeight() {
	...
	height += 10 + parseInt(doc.getElementById("tdControl").getAttribute("height")) + 8;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppIdecAd1", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppIdecAd1", "shortMenu", short);
	setSi(srcdoc, "StatAppIdecAd1", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppIdecAd1", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppIdecAd1", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagIdecAd1", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagIdecAd1", "MrlAppHlp"));
};
...
function changeHeight(pnlshort, height) {
	...
	if (pnlshort == "Control") {
		doc.getElementById("tdControl").setAttribute("height", "" + height);
		doc.getElementById("Control").setAttribute("height", "" + height);
	};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecAd1Do", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecAd1Do", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfIdecAd1", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecAd1Data", "ContInfIdecAd1", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecAd1Data", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngIdecAd1Data", "StatAppIdecAd1", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngIdecAd1Data", "StatShrIdecAd1", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecAd1Data", "TagIdecAd1", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecAd1Data") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefControl = retrieveSi(srcdoc, "StatShrIdecAd1", "scrJrefControl");
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrIdecAd1", "scrJrefHeadbar");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecAd1Data") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxIdecVReqitmode = retrieveSi(srcdoc, "StatAppIdecAd1", "srefIxIdecVReqitmode");

	if (srefIxIdecVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxIdecVReqitmode = "notify";
		setSi(srcdoc, "StatAppIdecAd1", "srefIxIdecVReqitmode", srefIxIdecVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxIdecVReqitmode = retrieveSi(srcdoc, "StatAppIdecAd1", "srefIxIdecVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppIdecAd1", "latency"));
			...
			if (blk) {
				if (blk.nodeName == "DpchEngIdecAck") {
					...
					if (blk.nodeName == "DpchEngIdecAlert") {
						...
						} else if (_scrJref == scrJrefControl) {
							doc.getElementById("Control").contentWindow.handleDpchEng(dom, blk.nodeName);
