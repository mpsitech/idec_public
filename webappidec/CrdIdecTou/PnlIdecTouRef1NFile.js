/**
  * \file PnlIdecTouRef1NFile.js
  * web client functionality for panel PnlIdecTouRef1NFile
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppIdecTouRef1NFile", "srefIxIdecVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppIdecTouRef1NFile", "srefIxIdecVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlIdecTouRef1NFile_aside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlIdecTouRef1NFile_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppIdecTouRef1NFile", "srefIxIdecVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppIdecTouRef1NFile", "srefIxIdecVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlIdecTouRef1NFile_bside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlIdecTouRef1NFile_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecTouRef1NFile", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagIdecTouRef1NFile", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxIdecVExpstate = retrieveSi(srcdoc, "StatAppIdecTouRef1NFile", "srefIxIdecVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecTouRef1NFile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryIdecTouRef1NFile", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryIdecTouRef1NFile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecTouRef1NFile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecTouRef1NFile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecTouRef1NFile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecTouRef1NFile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryIdecTouRef1NFile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecTouRef1NFile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecTouRef1NFile", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecTouRef1NFile", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagIdecTouRef1NFile", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryIdecTouRef1NFile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListIdecQTouRef1NFile
		rownode = getRownode(srcdoc, "ListIdecQTouRef1NFile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagIdecTouRef1NFile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagIdecTouRef1NFile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfIdecTouRef1NFile", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecTouRef1NFile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecTouRef1NFile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecTouRef1NFile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryIdecTouRef1NFile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryIdecTouRef1NFile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecTouRef1NFileData", scrJref, "StgIacQryIdecTouRef1NFile");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppIdecTouRef1NFile", "srefIxIdecVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecTouRef1NFile", "firstcol"));

	setSi(srcdoc, "StatAppQryIdecTouRef1NFile", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppIdecTouRef1NFile", "srefIxIdecVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecTouRef1NFile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecTouRef1NFileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecTouRef1NFileData", "ContInfIdecTouRef1NFile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecTouRef1NFileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngIdecTouRef1NFileData", "StatAppIdecTouRef1NFile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngIdecTouRef1NFileData", "StatShrIdecTouRef1NFile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecTouRef1NFileData", "StgIacIdecTouRef1NFile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngIdecTouRef1NFileData", "TagIdecTouRef1NFile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngIdecTouRef1NFileData", "ListIdecQTouRef1NFile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngIdecTouRef1NFileData", "StatAppQryIdecTouRef1NFile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngIdecTouRef1NFileData", "StatShrQryIdecTouRef1NFile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngIdecTouRef1NFileData", "StgIacQryIdecTouRef1NFile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngIdecTouRef1NFileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecTouRef1NFileData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecTouRef1NFileData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
