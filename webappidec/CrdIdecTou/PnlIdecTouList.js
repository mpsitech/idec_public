/**
  * \file PnlIdecTouList.js
  * web client functionality for panel PnlIdecTouList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlIdecTouList_aside.html";
	doc.getElementById("List_cont").src = "./PnlIdecTouList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlIdecTouList_bside.html";
	doc.getElementById("List_cont").src = "./PnlIdecTouList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecTouList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagIdecTouList", "Trs"));
...
function init() {
	var srefIxIdecVExpstate = retrieveSi(srcdoc, "StatShrIdecTouList", "srefIxIdecVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfIdecTouList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagIdecTouList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecTouList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryIdecTouList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecTouList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfIdecTouList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryIdecTouList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryIdecTouList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecTouList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecTouList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecTouList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecTouList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryIdecTouList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagIdecTouList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfIdecTouList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecTouList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecTouList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecTouList", "TcoTitWidth"));
		var TcoMisWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecTouList", "TcoMisWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecTouList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecTouList", "TcoStoWidth"));
		var TcoAreWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecTouList", "TcoAreWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacIdecTouList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoMis"); doc.widths.push(TcoMisWidth); doc.fs.push("mis"); doc.tos.push(2); wtot += TcoMisWidth;
			} else if (i == 3) {
				...
				doc.cols.push("TcoAre"); doc.widths.push(TcoAreWidth); doc.fs.push("are"); doc.tos.push(0); wtot += TcoAreWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagIdecTouList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryIdecTouList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListIdecQTouList
		rownode = getRownode(srcdoc, "ListIdecQTouList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagIdecTouList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagIdecTouList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfIdecTouList", "numFCsiQst"));
...
function refresh(chgcol) {
	...
	else if (srefIxIdecVExpstate == "regd") refreshB(chgcol);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecTouList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecTouList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecTouList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryIdecTouList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryIdecTouList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecTouListData", scrJref, "StgIacQryIdecTouList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecTouList", "firstcol"));

	setSi(srcdoc, "StatAppQryIdecTouList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecTouList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecTouListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacIdecTouList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacIdecTouList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecTouListData", scrJref, "ContIacIdecTouList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecTouListData", "ContIacIdecTouList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngIdecTouListData", "ContInfIdecTouList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecTouListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngIdecTouListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngIdecTouListData", "StatShrIdecTouList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecTouListData", "StgIacIdecTouList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngIdecTouListData", "TagIdecTouList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngIdecTouListData", "ListIdecQTouList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngIdecTouListData", "StatAppQryIdecTouList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngIdecTouListData", "StatShrQryIdecTouList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngIdecTouListData", "StgIacQryIdecTouList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngIdecTouListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecTouListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecTouListData") {
				mask = mergeDpchEngData(dom);
