/**
  * \file CrdIdecTou.js
  * web client functionality for card CrdIdecTou
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppIdecTou", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppIdecTou", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppIdecTou", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppIdecTou", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppIdecTou", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlIdecTouHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlIdecTouList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlIdecTouRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppIdecTou", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppIdecTou", "shortMenu", short);
	setSi(srcdoc, "StatAppIdecTou", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppIdecTou", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppIdecTou", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagIdecTou", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagIdecTou", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrIdecTou", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrIdecTou", "MitCrdNewAvail") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppIdecTou", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdTou"), retrieveCi(srcdoc, "ContInfIdecTou", "MtxCrdTou"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagIdecTou", "MitCrdNew"));
...
function refresh() {
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrIdecTou", "scrJrefDlgnew");

	if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgIdecTouNew", scrJrefDlgnew);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfIdecTou", "MtxCrdTou") + " - ICARUSDetectorControl 0.2.1";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecTouDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecTouDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfIdecTou", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecTouData", "ContInfIdecTou", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecTouData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngIdecTouData", "StatAppIdecTou", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngIdecTouData", "StatShrIdecTou", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecTouData", "TagIdecTou", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecTouData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrIdecTou", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrIdecTou", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrIdecTou", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecTouData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxIdecVReqitmode = retrieveSi(srcdoc, "StatAppIdecTou", "srefIxIdecVReqitmode");

	if (srefIxIdecVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxIdecVReqitmode = "notify";
		setSi(srcdoc, "StatAppIdecTou", "srefIxIdecVReqitmode", srefIxIdecVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxIdecVReqitmode = retrieveSi(srcdoc, "StatAppIdecTou", "srefIxIdecVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppIdecTou", "latency"));
