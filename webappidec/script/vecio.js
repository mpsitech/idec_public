...
function parseUsmallintvec(str) {
	...
	if (buf) {
		buflen = buf.byteLength;
		if ((buflen%2) == 0) return new Uint16Array(buf);
	};
...
function parseIntvec(str) {
	...
	if (buf) {
		buflen = buf.byteLength;
		if ((buflen%4) == 0) return new Int32Array(buf);
	};
...
function parseUintvec(str) {
	...
	if (buf) {
		buflen = buf.byteLength;
		if ((buflen%4) == 0) return new Uint32Array(buf);
	};
...
function parseFloatvec(str) {
	...
	if (buf) {
		buflen = buf.byteLength;
		if ((buflen%4) == 0) return new Float32Array(buf);
	};
...
function parseDoublevec(str) {
	...
	if (buf) {
		buflen = buf.byteLength;
		if ((buflen%8) == 0) return new Float64Array(buf);
	};
