/**
  * \file CrdIdecMis.js
  * web client functionality for card CrdIdecMis
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppIdecMis", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppIdecMis", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppIdecMis", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppIdecMis", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppIdecMis", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlIdecMisHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlIdecMisList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlIdecMisRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppIdecMis", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppIdecMis", "shortMenu", short);
	setSi(srcdoc, "StatAppIdecMis", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppIdecMis", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppIdecMis", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagIdecMis", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagIdecMis", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrIdecMis", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrIdecMis", "MitCrdNewAvail") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppIdecMis", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdMis"), retrieveCi(srcdoc, "ContInfIdecMis", "MtxCrdMis"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagIdecMis", "MitCrdNew"));
...
function refresh() {
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrIdecMis", "scrJrefDlgnew");

	if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgIdecMisNew", scrJrefDlgnew);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfIdecMis", "MtxCrdMis") + " - ICARUSDetectorControl 0.2.1";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecMisDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecMisDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfIdecMis", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecMisData", "ContInfIdecMis", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecMisData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngIdecMisData", "StatAppIdecMis", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngIdecMisData", "StatShrIdecMis", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecMisData", "TagIdecMis", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecMisData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrIdecMis", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrIdecMis", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrIdecMis", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecMisData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxIdecVReqitmode = retrieveSi(srcdoc, "StatAppIdecMis", "srefIxIdecVReqitmode");

	if (srefIxIdecVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxIdecVReqitmode = "notify";
		setSi(srcdoc, "StatAppIdecMis", "srefIxIdecVReqitmode", srefIxIdecVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxIdecVReqitmode = retrieveSi(srcdoc, "StatAppIdecMis", "srefIxIdecVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppIdecMis", "latency"));
