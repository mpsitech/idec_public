/**
  * \file PnlIdecMis1NTour.js
  * web client functionality for panel PnlIdecMis1NTour
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppIdecMis1NTour", "srefIxIdecVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppIdecMis1NTour", "srefIxIdecVExpstate", "mind");

	getRecwnd().changeHeight("1NTour", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NTour_side").setAttribute("height", "30");
	doc.getElementById("1NTour_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NTour_side").src = "./PnlIdecMis1NTour_aside.html";
	doc.getElementById("1NTour_cont").src = "./PnlIdecMis1NTour_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppIdecMis1NTour", "srefIxIdecVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppIdecMis1NTour", "srefIxIdecVExpstate", "regd");

	getRecwnd().changeHeight("1NTour", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NTour_side").setAttribute("height", "255");
	doc.getElementById("1NTour_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NTour_side").src = "./PnlIdecMis1NTour_bside.html";
	doc.getElementById("1NTour_cont").src = "./PnlIdecMis1NTour_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NTour_side").contentDocument;
	contdoc = doc.getElementById("1NTour_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NTour_side").contentDocument;
	contdoc = doc.getElementById("1NTour_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NTour_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NTour_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NTour_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecMis1NTour", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagIdecMis1NTour", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxIdecVExpstate = retrieveSi(srcdoc, "StatAppIdecMis1NTour", "srefIxIdecVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecMis1NTour", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryIdecMis1NTour", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryIdecMis1NTour", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecMis1NTour", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecMis1NTour", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecMis1NTour", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecMis1NTour", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryIdecMis1NTour", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecMis1NTour", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecMis1NTour", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecMis1NTour", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagIdecMis1NTour", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryIdecMis1NTour", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListIdecQMis1NTour
		rownode = getRownode(srcdoc, "ListIdecQMis1NTour", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagIdecMis1NTour", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagIdecMis1NTour", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfIdecMis1NTour", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NTour", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NTour_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NTour_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NTour_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NTour_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecMis1NTour", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecMis1NTour", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecMis1NTour", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryIdecMis1NTour", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryIdecMis1NTour", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecMis1NTourData", scrJref, "StgIacQryIdecMis1NTour");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppIdecMis1NTour", "srefIxIdecVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecMis1NTour", "firstcol"));

	setSi(srcdoc, "StatAppQryIdecMis1NTour", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppIdecMis1NTour", "srefIxIdecVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecMis1NTour", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecMis1NTourDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecMis1NTourData", "ContInfIdecMis1NTour", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecMis1NTourData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngIdecMis1NTourData", "StatAppIdecMis1NTour", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngIdecMis1NTourData", "StatShrIdecMis1NTour", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecMis1NTourData", "StgIacIdecMis1NTour", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngIdecMis1NTourData", "TagIdecMis1NTour", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngIdecMis1NTourData", "ListIdecQMis1NTour", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngIdecMis1NTourData", "StatAppQryIdecMis1NTour", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngIdecMis1NTourData", "StatShrQryIdecMis1NTour", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngIdecMis1NTourData", "StgIacQryIdecMis1NTour", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngIdecMis1NTourData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecMis1NTourData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NTour");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecMis1NTourData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
