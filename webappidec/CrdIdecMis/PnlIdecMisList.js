/**
  * \file PnlIdecMisList.js
  * web client functionality for panel PnlIdecMisList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlIdecMisList_aside.html";
	doc.getElementById("List_cont").src = "./PnlIdecMisList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlIdecMisList_bside.html";
	doc.getElementById("List_cont").src = "./PnlIdecMisList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecMisList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagIdecMisList", "Trs"));
...
function init() {
	var srefIxIdecVExpstate = retrieveSi(srcdoc, "StatShrIdecMisList", "srefIxIdecVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecMisList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryIdecMisList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryIdecMisList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecMisList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecMisList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecMisList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecMisList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryIdecMisList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecMisList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecMisList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecMisList", "TcoTitWidth"));
		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecMisList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecMisList", "TcoOwnWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecMisList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecMisList", "TcoStoWidth"));
		var TcoCryWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecMisList", "TcoCryWidth"));
		var TcoRegWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecMisList", "TcoRegWidth"));
		var TcoMunWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecMisList", "TcoMunWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacIdecMisList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoCry"); doc.widths.push(TcoCryWidth); doc.fs.push("cry2"); doc.tos.push(0); wtot += TcoCryWidth;
			} else if (i == 7) {
				doc.cols.push("TcoReg"); doc.widths.push(TcoRegWidth); doc.fs.push("reg"); doc.tos.push(0); wtot += TcoRegWidth;
			} else if (i == 8) {
				doc.cols.push("TcoMun"); doc.widths.push(TcoMunWidth); doc.fs.push("mun"); doc.tos.push(0); wtot += TcoMunWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagIdecMisList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryIdecMisList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListIdecQMisList
		rownode = getRownode(srcdoc, "ListIdecQMisList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagIdecMisList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagIdecMisList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfIdecMisList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecMisList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecMisList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecMisList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryIdecMisList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryIdecMisList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecMisListData", scrJref, "StgIacQryIdecMisList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecMisList", "firstcol"));

	setSi(srcdoc, "StatAppQryIdecMisList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecMisList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecMisListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacIdecMisList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacIdecMisList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecMisListData", scrJref, "ContIacIdecMisList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecMisListData", "ContIacIdecMisList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngIdecMisListData", "ContInfIdecMisList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecMisListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngIdecMisListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngIdecMisListData", "StatShrIdecMisList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecMisListData", "StgIacIdecMisList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngIdecMisListData", "TagIdecMisList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngIdecMisListData", "ListIdecQMisList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngIdecMisListData", "StatAppQryIdecMisList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngIdecMisListData", "StatShrQryIdecMisList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngIdecMisListData", "StgIacQryIdecMisList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngIdecMisListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecMisListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecMisListData") {
				mask = mergeDpchEngData(dom);
