/**
  * \file PnlIdecUsr1NSession.js
  * web client functionality for panel PnlIdecUsr1NSession
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppIdecUsr1NSession", "srefIxIdecVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppIdecUsr1NSession", "srefIxIdecVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NSession_side").src = "./PnlIdecUsr1NSession_aside.html";
	doc.getElementById("1NSession_cont").src = "./PnlIdecUsr1NSession_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppIdecUsr1NSession", "srefIxIdecVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppIdecUsr1NSession", "srefIxIdecVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NSession_side").src = "./PnlIdecUsr1NSession_bside.html";
	doc.getElementById("1NSession_cont").src = "./PnlIdecUsr1NSession_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecUsr1NSession", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagIdecUsr1NSession", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxIdecVExpstate = retrieveSi(srcdoc, "StatAppIdecUsr1NSession", "srefIxIdecVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecUsr1NSession", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryIdecUsr1NSession", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsr1NSession", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsr1NSession", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsr1NSession", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsr1NSession", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsr1NSession", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsr1NSession", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsr1NSession", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsr1NSession", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecUsr1NSession", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagIdecUsr1NSession", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryIdecUsr1NSession", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListIdecQUsr1NSession
		rownode = getRownode(srcdoc, "ListIdecQUsr1NSession", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagIdecUsr1NSession", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagIdecUsr1NSession", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfIdecUsr1NSession", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsr1NSession", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsr1NSession", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsr1NSession", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryIdecUsr1NSession", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryIdecUsr1NSession", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecUsr1NSessionData", scrJref, "StgIacQryIdecUsr1NSession");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppIdecUsr1NSession", "srefIxIdecVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsr1NSession", "firstcol"));

	setSi(srcdoc, "StatAppQryIdecUsr1NSession", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppIdecUsr1NSession", "srefIxIdecVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsr1NSession", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecUsr1NSessionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecUsr1NSessionData", "ContInfIdecUsr1NSession", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecUsr1NSessionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngIdecUsr1NSessionData", "StatAppIdecUsr1NSession", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngIdecUsr1NSessionData", "StatShrIdecUsr1NSession", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecUsr1NSessionData", "StgIacIdecUsr1NSession", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngIdecUsr1NSessionData", "TagIdecUsr1NSession", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngIdecUsr1NSessionData", "ListIdecQUsr1NSession", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngIdecUsr1NSessionData", "StatAppQryIdecUsr1NSession", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngIdecUsr1NSessionData", "StatShrQryIdecUsr1NSession", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngIdecUsr1NSessionData", "StgIacQryIdecUsr1NSession", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngIdecUsr1NSessionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecUsr1NSessionData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecUsr1NSessionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
