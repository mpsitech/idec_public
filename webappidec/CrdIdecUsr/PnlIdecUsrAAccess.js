/**
  * \file PnlIdecUsrAAccess.js
  * web client functionality for panel PnlIdecUsrAAccess
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppIdecUsrAAccess", "srefIxIdecVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppIdecUsrAAccess", "srefIxIdecVExpstate", "mind");
	...
	// change content
	doc.getElementById("AAccess_side").src = "./PnlIdecUsrAAccess_aside.html";
	doc.getElementById("AAccess_cont").src = "./PnlIdecUsrAAccess_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppIdecUsrAAccess", "srefIxIdecVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppIdecUsrAAccess", "srefIxIdecVExpstate", "regd");
	...
	// change content
	doc.getElementById("AAccess_side").src = "./PnlIdecUsrAAccess_bside.html";
	doc.getElementById("AAccess_cont").src = "./PnlIdecUsrAAccess_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecUsrAAccess", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagIdecUsrAAccess", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxIdecVExpstate = retrieveSi(srcdoc, "StatAppIdecUsrAAccess", "srefIxIdecVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecUsrAAccess", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryIdecUsrAAccess", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsrAAccess", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsrAAccess", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsrAAccess", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsrAAccess", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsrAAccess", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsrAAccess", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsrAAccess", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsrAAccess", "ncol"));

		var TcoCrdWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecUsrAAccess", "TcoCrdWidth"));
		var TcoUacWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecUsrAAccess", "TcoUacWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagIdecUsrAAccess", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryIdecUsrAAccess", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListIdecQUsrAAccess
		rownode = getRownode(srcdoc, "ListIdecQUsrAAccess", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagIdecUsrAAccess", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagIdecUsrAAccess", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfIdecUsrAAccess", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsrAAccess", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsrAAccess", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsrAAccess", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryIdecUsrAAccess", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryIdecUsrAAccess", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecUsrAAccessData", scrJref, "StgIacQryIdecUsrAAccess");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppIdecUsrAAccess", "srefIxIdecVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsrAAccess", "firstcol"));

	setSi(srcdoc, "StatAppQryIdecUsrAAccess", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppIdecUsrAAccess", "srefIxIdecVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsrAAccess", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecUsrAAccessDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecUsrAAccessData", "ContInfIdecUsrAAccess", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecUsrAAccessData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngIdecUsrAAccessData", "StatAppIdecUsrAAccess", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngIdecUsrAAccessData", "StatShrIdecUsrAAccess", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecUsrAAccessData", "StgIacIdecUsrAAccess", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngIdecUsrAAccessData", "TagIdecUsrAAccess", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngIdecUsrAAccessData", "ListIdecQUsrAAccess", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngIdecUsrAAccessData", "StatAppQryIdecUsrAAccess", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngIdecUsrAAccessData", "StatShrQryIdecUsrAAccess", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngIdecUsrAAccessData", "StgIacQryIdecUsrAAccess", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngIdecUsrAAccessData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecUsrAAccessData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecUsrAAccessData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
