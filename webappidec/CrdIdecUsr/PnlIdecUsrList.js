/**
  * \file PnlIdecUsrList.js
  * web client functionality for panel PnlIdecUsrList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlIdecUsrList_aside.html";
	doc.getElementById("List_cont").src = "./PnlIdecUsrList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlIdecUsrList_bside.html";
	doc.getElementById("List_cont").src = "./PnlIdecUsrList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecUsrList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagIdecUsrList", "Trs"));
...
function init() {
	var srefIxIdecVExpstate = retrieveSi(srcdoc, "StatShrIdecUsrList", "srefIxIdecVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagIdecUsrList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryIdecUsrList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsrList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsrList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsrList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsrList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsrList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsrList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsrList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsrList", "ncol"));

		var TcoPrsWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecUsrList", "TcoPrsWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecUsrList", "TcoSrfWidth"));
		var TcoUsgWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecUsrList", "TcoUsgWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecUsrList", "TcoSteWidth"));
		var TcoLclWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecUsrList", "TcoLclWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacIdecUsrList", "TcoUlvWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacIdecUsrList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoSte"); doc.widths.push(TcoSteWidth); doc.fs.push("ste"); doc.tos.push(4); wtot += TcoSteWidth;
			} else if (i == 5) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagIdecUsrList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryIdecUsrList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListIdecQUsrList
		rownode = getRownode(srcdoc, "ListIdecQUsrList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagIdecUsrList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagIdecUsrList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfIdecUsrList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsrList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsrList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryIdecUsrList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryIdecUsrList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryIdecUsrList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecUsrListData", scrJref, "StgIacQryIdecUsrList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryIdecUsrList", "firstcol"));

	setSi(srcdoc, "StatAppQryIdecUsrList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryIdecUsrList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecUsrListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacIdecUsrList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacIdecUsrList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppIdecUsrListData", scrJref, "ContIacIdecUsrList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecUsrListData", "ContIacIdecUsrList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngIdecUsrListData", "ContInfIdecUsrList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecUsrListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngIdecUsrListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngIdecUsrListData", "StatShrIdecUsrList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecUsrListData", "StgIacIdecUsrList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngIdecUsrListData", "TagIdecUsrList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngIdecUsrListData", "ListIdecQUsrList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngIdecUsrListData", "StatAppQryIdecUsrList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngIdecUsrListData", "StatShrQryIdecUsrList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngIdecUsrListData", "StgIacQryIdecUsrList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngIdecUsrListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecUsrListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecUsrListData") {
				mask = mergeDpchEngData(dom);
