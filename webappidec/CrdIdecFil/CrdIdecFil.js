/**
  * \file CrdIdecFil.js
  * web client functionality for card CrdIdecFil
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppIdecFil", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppIdecFil", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppIdecFil", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppIdecFil", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppIdecFil", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlIdecFilHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlIdecFilList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlIdecFilRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppIdecFil", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppIdecFil", "shortMenu", short);
	setSi(srcdoc, "StatAppIdecFil", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppIdecFil", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppIdecFil", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagIdecFil", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagIdecFil", "MrlAppHlp"));
};

function initMenCrd() {
	var mendoc = doc.getElementById("Menu").contentDocument;

	MitCrdExmActive = (retrieveSi(srcdoc, "StatShrIdecFil", "MitCrdExmActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppIdecFil", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdFil"), retrieveCi(srcdoc, "ContInfIdecFil", "MtxCrdFil"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdExm"), retrieveTi(srcdoc, "TagIdecFil", "MitCrdExm"));
	setMitActive("MitCrdExm", MitCrdExmActive);
};
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgIdecFilExtrmed")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgextrmed = retrieveSi(srcdoc, "StatShrIdecFil", "scrJrefDlgextrmed");

	if (scrJrefDlgextrmed != "") {
		if (scrJrefDlg != scrJrefDlgextrmed) showDlg("DlgIdecFilExtrmed", scrJrefDlgextrmed);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfIdecFil", "MtxCrdFil") + " - ICARUSDetectorControl 0.2.1";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecFilDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppIdecFilDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfIdecFil", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngIdecFilData", "ContInfIdecFil", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngIdecFilData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngIdecFilData", "StatAppIdecFil", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngIdecFilData", "StatShrIdecFil", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngIdecFilData", "TagIdecFil", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngIdecFilData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrIdecFil", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrIdecFil", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrIdecFil", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngIdecFilData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxIdecVReqitmode = retrieveSi(srcdoc, "StatAppIdecFil", "srefIxIdecVReqitmode");

	if (srefIxIdecVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxIdecVReqitmode = "notify";
		setSi(srcdoc, "StatAppIdecFil", "srefIxIdecVReqitmode", srefIxIdecVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxIdecVReqitmode = retrieveSi(srcdoc, "StatAppIdecFil", "srefIxIdecVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppIdecFil", "latency"));
