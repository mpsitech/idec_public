/**
  * \file PnlIdecTouRec_blks.cpp
  * job handler for job PnlIdecTouRec (implementation of blocks)
...
void PnlIdecTouRec::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfIdecTouRec";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfIdecTouRec";
...
void PnlIdecTouRec::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppIdecTouRec";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppIdecTouRec";
...
PnlIdecTouRec::StatShr::StatShr(
	...
	mask = {IXIDECVEXPSTATE, JREFDETAIL, JREFREF1NFILE, BUTREGULARIZEACTIVE};
};

void PnlIdecTouRec::StatShr::writeXML(
	...
	if (difftag.length() == 0) difftag = "StatShrIdecTouRec";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrIdecTouRec";
...
set<uint> PnlIdecTouRec::StatShr::diff(
	...
	diffitems = {IXIDECVEXPSTATE, JREFDETAIL, JREFREF1NFILE, BUTREGULARIZEACTIVE};
	for (auto it=commitems.begin();it!=commitems.end();it++) diffitems.erase(*it);
...
void PnlIdecTouRec::Tag::writeXML(
			const uint ixIdecVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagIdecTouRec";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemIdecTouRec";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixIdecVLocale == VecIdecVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Tour");
...
PnlIdecTouRec::DpchAppDo::DpchAppDo() : DpchAppIdec(VecIdecVDpch::DPCHAPPIDECTOURECDO) {
	ixVDo = 0;
};
...
void PnlIdecTouRec::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppIdecTouRecDo");
	else
...
PnlIdecTouRec::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngIdec(VecIdecVDpch::DPCHENGIDECTOURECDATA, jref) {
...
void PnlIdecTouRec::DpchEngData::merge(
			DpchEngIdec* dpcheng
		) {
...
void PnlIdecTouRec::DpchEngData::writeXML(
			const uint ixIdecVLocale
			, pthread_mutex_t* mScr
			, map<ubigint,string>& scr
			, map<string,ubigint>& descr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngIdecTouRecData");
