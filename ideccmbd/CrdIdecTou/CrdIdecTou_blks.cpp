/**
  * \file CrdIdecTou_blks.cpp
  * job handler for job CrdIdecTou (implementation of blocks)
...
void CrdIdecTou::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void CrdIdecTou::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfIdecTou";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfIdecTou";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		writeUintAttr(wr, itemtag, "sref", "numFSge", numFSge);
		writeStringAttr(wr, itemtag, "sref", "MrlAppHlp", MrlAppHlp);
		writeStringAttr(wr, itemtag, "sref", "MtxCrdTou", MtxCrdTou);
...
set<uint> CrdIdecTou::ContInf::comm(
	...
	if (MtxCrdTou == comp->MtxCrdTou) insert(items, MTXCRDTOU);
...
void CrdIdecTou::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppIdecTou";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppIdecTou";
...
void CrdIdecTou::StatShr::writeXML(
	...
	if (difftag.length() == 0) difftag = "StatShrIdecTou";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrIdecTou";
...
void CrdIdecTou::Tag::writeXML(
			const uint ixIdecVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagIdecTou";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemIdecTou";
...
CrdIdecTou::DpchAppDo::DpchAppDo() : DpchAppIdec(VecIdecVDpch::DPCHAPPIDECTOUDO) {
	ixVDo = 0;
};
...
void CrdIdecTou::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppIdecTouDo");
	else
...
CrdIdecTou::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFSge
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngIdec(VecIdecVDpch::DPCHENGIDECTOUDATA, jref) {
...
void CrdIdecTou::DpchEngData::merge(
			DpchEngIdec* dpcheng
		) {
...
void CrdIdecTou::DpchEngData::writeXML(
			const uint ixIdecVLocale
			, pthread_mutex_t* mScr
			, map<ubigint,string>& scr
			, map<string,ubigint>& descr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngIdecTouData");
