/**
  ...
  * job handler for job PnlIdecUsgMNUser (implementation)
...
#include "PnlIdecUsgMNUser_blks.cpp"
#include "PnlIdecUsgMNUser_evals.cpp"

/******************************************************************************
 class PnlIdecUsgMNUser
 ******************************************************************************/

PnlIdecUsgMNUser::PnlIdecUsgMNUser(
			XchgIdec* xchg
			, DbsIdec* dbsidec
			, const ubigint jrefSup
			, const uint ixIdecVLocale
		) : JobIdec(xchg, VecIdecVJob::PNLIDECUSGMNUSER, jrefSup, ixIdecVLocale) {
	...
	qry = new QryIdecUsgMNUser(xchg, dbsidec, jref, ixIdecVLocale);
...
// IP cust --- INSERT
DpchEngIdec* PnlIdecUsgMNUser::getNewDpchEng(
			set<uint> items
		) {
...
void PnlIdecUsgMNUser::updatePreset(
			DbsIdec* dbsidec
...
void PnlIdecUsgMNUser::handleRequest(
			DbsIdec* dbsidec
			, ReqIdec* req
		) {
	if (req->ixVBasetype == ReqIdec::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixIdecVDpch == VecIdecVDpch::DPCHAPPIDECUSGMNUSERDATA) {
		...
		} else if (req->dpchapp->ixIdecVDpch == VecIdecVDpch::DPCHAPPIDECUSGMNUSERDO) {
...
void PnlIdecUsgMNUser::handleDpchAppIdecInit(
			DbsIdec* dbsidec
...
void PnlIdecUsgMNUser::handleDpchAppDataStgiac(
			DbsIdec* dbsidec
			, StgIac* _stgiac
			, DpchEngIdec** dpcheng
		) {
...
void PnlIdecUsgMNUser::handleDpchAppDataStgiacqry(
			DbsIdec* dbsidec
			, QryIdecUsgMNUser::StgIac* _stgiacqry
			, DpchEngIdec** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryIdecUsgMNUser::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryIdecUsgMNUser::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsidec);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryIdecUsgMNUser::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbsidec->tblidecmuser->loadRecByRef(recSelNew->mref, &_recUsr)) {
					recUsr = *_recUsr;
					delete _recUsr;
				};
			};
...
void PnlIdecUsgMNUser::handleDpchAppDoButViewClick(
			DbsIdec* dbsidec
			, DpchEngIdec** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecIdecVPreset::PREIDECIXUACUSR, jref)) {
			sref = "CrdIdecUsr";
			xchg->triggerIxRefSrefIntvalToRefCall(dbsidec, VecIdecVCall::CALLIDECCRDOPEN, jref, 0, 0, sref, recUsr.ref, jrefNew);
		};
...
void PnlIdecUsgMNUser::handleDpchAppDoButAddClick(
			DbsIdec* dbsidec
			, DpchEngIdec** dpcheng
		) {
...
void PnlIdecUsgMNUser::handleDpchAppDoButSubClick(
			DbsIdec* dbsidec
			, DpchEngIdec** dpcheng
		) {
...
void PnlIdecUsgMNUser::handleDpchAppDoButRefreshClick(
			DbsIdec* dbsidec
			, DpchEngIdec** dpcheng
		) {
	set<uint> moditems;
...
void PnlIdecUsgMNUser::handleCall(
			DbsIdec* dbsidec
			, Call* call
		) {
...
bool PnlIdecUsgMNUser::handleCallIdecStatChg(
			DbsIdec* dbsidec
			, const ubigint jrefTrig
		) {
	bool retval = false;
