/**
  * \file CrdIdecUsg_blks.cpp
  * job handler for job CrdIdecUsg (implementation of blocks)
...
void CrdIdecUsg::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void CrdIdecUsg::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfIdecUsg";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfIdecUsg";
...
void CrdIdecUsg::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppIdecUsg";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppIdecUsg";
...
void CrdIdecUsg::StatShr::writeXML(
	...
	if (difftag.length() == 0) difftag = "StatShrIdecUsg";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrIdecUsg";
...
void CrdIdecUsg::Tag::writeXML(
			const uint ixIdecVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagIdecUsg";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemIdecUsg";
...
CrdIdecUsg::DpchAppDo::DpchAppDo() : DpchAppIdec(VecIdecVDpch::DPCHAPPIDECUSGDO) {
	ixVDo = 0;
};
...
void CrdIdecUsg::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppIdecUsgDo");
	else
...
CrdIdecUsg::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFSge
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngIdec(VecIdecVDpch::DPCHENGIDECUSGDATA, jref) {
...
void CrdIdecUsg::DpchEngData::merge(
			DpchEngIdec* dpcheng
		) {
...
void CrdIdecUsg::DpchEngData::writeXML(
			const uint ixIdecVLocale
			, pthread_mutex_t* mScr
			, map<ubigint,string>& scr
			, map<string,ubigint>& descr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngIdecUsgData");
