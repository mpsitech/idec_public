/**
  * \file PnlIdecUsgDetail_blks.cpp
  * job handler for job PnlIdecUsgDetail (implementation of blocks)
...
void PnlIdecUsgDetail::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfIdecUsgDetail";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfIdecUsgDetail";
...
void PnlIdecUsgDetail::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixIdecVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppIdecUsgDetail";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppIdecUsgDetail";
...
void PnlIdecUsgDetail::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrIdecUsgDetail";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrIdecUsgDetail";
...
void PnlIdecUsgDetail::Tag::writeXML(
			const uint ixIdecVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagIdecUsgDetail";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemIdecUsgDetail";
...
PnlIdecUsgDetail::DpchAppData::DpchAppData() : DpchAppIdec(VecIdecVDpch::DPCHAPPIDECUSGDETAILDATA) {
};
...
void PnlIdecUsgDetail::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppIdecUsgDetailData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlIdecUsgDetail::DpchAppDo::DpchAppDo() : DpchAppIdec(VecIdecVDpch::DPCHAPPIDECUSGDETAILDO) {
	ixVDo = 0;
};
...
void PnlIdecUsgDetail::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppIdecUsgDetailDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlIdecUsgDetail::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngIdec(VecIdecVDpch::DPCHENGIDECUSGDETAILDATA, jref) {
...
void PnlIdecUsgDetail::DpchEngData::merge(
			DpchEngIdec* dpcheng
		) {
...
void PnlIdecUsgDetail::DpchEngData::writeXML(
			const uint ixIdecVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngIdecUsgDetailData");
