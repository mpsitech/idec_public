/**
  * \file IdeccmbdJobprc.cpp
  * job processor for Idec combined daemon (implementation)
...
void* IdeccmbdJobprc::run(
	...
	while (true) {
		...
		while (req == NULL) {
			...
			Mutex::lock(&(xchg->mcJobprcs), "xchg->mcJobprcs", "IdeccmbdJobprc", "run");
			Cond::wait(&(xchg->cJobprcs), &(xchg->mcJobprcs), "xchg->cJobprcs", "IdeccmbdJobprc", "run");

			req = xchg->pullFirstReq();

			Mutex::unlock(&(xchg->mcJobprcs), "xchg->mcJobprcs", "IdeccmbdJobprc", "run");
		};

		// re-signal condition to avoid using broadcast
		Cond::signal(&(xchg->cJobprcs), &(xchg->mcJobprcs), "xchg->cJobprcs", "xchg->mcJobprcs", "IdeccmbdJobprc", "run");
		...
		if (req->ixVBasetype == ReqIdec::VecVBasetype::CMD) {
			...
			if (req->dpchapp) {
				//cout << "IdeccmbdJobprc::run() REGULAR request: '" << VecIdecVDpch::getSref(req->dpchapp->ixIdecVDpch) << "'" << endl;
			...
			//cout << "IdeccmbdJobprc::run() UPLOAD request file name: '" << req->filename << "'" << endl;
			...
			if (req->dpchret) {
				//cout << "IdeccmbdJobprc::run() RET request: '" << VecIdecVDpch::getSref(req->dpchret->ixIdecVDpch) << "'" << endl;
...
void IdeccmbdJobprc::cleanup(
			void* arg
		) {
	XchgIdeccmbd* xchg = (XchgIdeccmbd*) arg;

	Mutex::unlock(&(xchg->mcJobprcs), "xchg->mcJobprcs", "IdeccmbdJobprc", "cleanup");
};

void IdeccmbdJobprc::accessJob(
			XchgIdeccmbd* xchg
			, DbsIdec* dbsidec
			, ReqIdec* req
		) {
	...
	job = xchg->getJobByJref(req->jref);
	if (job) {
		if (!req->weak) job->lockAccess("IdeccmbdJobprc", "accessJob");
	...
	if (!req->weak) {
		job = xchg->getJobByJref(req->jref);
		if (job) job->unlockAccess("IdeccmbdJobprc", "accessJob");
	};
};
