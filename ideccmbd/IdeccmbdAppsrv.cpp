/**
  * \file IdeccmbdAppsrv.cpp
  * application server for Idec combined daemon (implementation)
...
MHD_Daemon* IdeccmbdAppsrv::start(
			XchgIdeccmbd* xchg
		) {
//	return MHD_start_daemon(MHD_USE_SELECT_INTERNALLY, xchg->stgideccmbd.appsrvport, &MhdAccept, (void*) xchg, &MhdCallback, (void*) xchg, MHD_OPTION_NOTIFY_COMPLETED, &MhdComplete, (void*) xchg, MHD_OPTION_THREAD_POOL_SIZE, 16, MHD_OPTION_END);
	return MHD_start_daemon(MHD_USE_THREAD_PER_CONNECTION, xchg->stgideccmbd.appsrvport, &MhdAccept, (void*) xchg, &MhdCallback, (void*) xchg, MHD_OPTION_NOTIFY_COMPLETED, &MhdComplete, (void*) xchg, MHD_OPTION_END);
};

void IdeccmbdAppsrv::stop(
			MHD_Daemon* d
		) {
	MHD_stop_daemon(d);
};

int IdeccmbdAppsrv::MhdAccept(
			void* cls
			, const sockaddr* addr
			, socklen_t addrlen
		) {
...
int IdeccmbdAppsrv::MhdCallback(
			void* cls
	...
	if (*con_cls == NULL) {
		...
		if (strcmp(method, "GET") == 0) {
			...
			if (s.find("/web/") != string::npos) {
				...
				// ex. "/html/CrdIdecNav/CrdIdecNav.html" ; note that eventual parameters such as "?scrJref=abcd1234efgh5678" are not included in url
			...
			*con_cls = req;
			...
			if (req->ixVBasetype == ReqIdec::VecVBasetype::WEB) {
				...
				if (valid) {
					...
					if (dcol->dpchs.empty()) {
						if (dcol->hot) {
							...
							Mutex::lock(&(req->mcReady), "req->mcReady", "IdeccmbdAppsrv", "MhdCallback[1]");
							dcol->unlockAccess("IdeccmbdAppsrv", "MhdCallback[3]");
							Cond::timedwait(&(req->cReady), &(req->mcReady), 15000000, "req->cReady", "IdeccmbdAppsrv", "MhdCallback");
							// cout << "got one after timedwait" << endl;
							Mutex::unlock(&(req->mcReady), "req->mcReady", "IdeccmbdAppsrv", "MhdCallback[1]");

							dcol = xchg->getDcolByJref(req->jref);
							if (dcol) {
								// note: the destructor of dcol is called after dcol is removed from the list from which it can be found by getDcolByJref()
								dcol->req = NULL;
								dcol->unlockAccess("IdeccmbdAppsrv", "MhdCallback[4]");
						...
						dcol->hot = true;
						dcol->unlockAccess("IdeccmbdAppsrv", "MhdCallback[5]");
				...
				if (valid) {
					...
					dcol->unlockAccess("IdeccmbdAppsrv", "MhdCallback[6]");
				...
				if (valid) {
					...
					// wait for reply
					Mutex::lock(&(req->mcReady), "req->mcReady", "IdeccmbdAppsrv", "MhdCallback[2]");
					Cond::wait(&(req->cReady), &(req->mcReady), "req->cReady", "IdeccmbdAppsrv", "MhdCallback[1]");
					Mutex::unlock(&(req->mcReady), "req->mcReady", "IdeccmbdAppsrv", "MhdCallback[2]");
				...
				*upload_data_size = 0;
				...
				if (req->ixVBasetype == ReqIdec::VecVBasetype::REGULAR) {
					...
					if (valid) {
						...
						// wait for reply
						Mutex::lock(&(req->mcReady), "req->mcReady", "IdeccmbdAppsrv", "MhdCallback[3]");
						if (req->ixVState != ReqIdec::VecVState::REPLY) Cond::wait(&(req->cReady), &(req->mcReady), "req->cReady", "IdeccmbdAppsrv", "MhdCallback[2]");
						Mutex::unlock(&(req->mcReady), "req->mcReady", "IdeccmbdAppsrv", "MhdCallback[3]");
					...
					if (valid) {
						...
						Mutex::lock(&(req->mcReady), "req->mcReady", "IdeccmbdAppsrv", "MhdCallback[4]");
						Cond::wait(&(req->cReady), &(req->mcReady), "req->cReady", "IdeccmbdAppsrv", "MhdCallback[3]");
						Mutex::unlock(&(req->mcReady), "req->mcReady", "IdeccmbdAppsrv", "MhdCallback[4]");
...
#ifdef _WIN32
int IdeccmbdAppsrv::MhdFilesend(
			void* con_cls
			, uint64_t pos
			, char* buf
			, int max
...
#else
ssize_t IdeccmbdAppsrv::MhdFilesend(
			void* con_cls
			, uint64_t pos
			, char* buf
			, size_t max
...
int IdeccmbdAppsrv::MhdPostrecv(
			void* con_cls
			, MHD_ValueKind kind
			, const char* key
			, const char* filename
...
void IdeccmbdAppsrv::MhdComplete(
			void* cls
			, MHD_Connection* connection
			, void** con_cls
...
bool IdeccmbdAppsrv::validateIp(
			XchgIdeccmbd* xchg
			, ReqIdec* req
		) {
	bool retval = true;
...
bool IdeccmbdAppsrv::validateJrefNotify(
			XchgIdeccmbd* xchg
			, ReqIdec* req
		) {
	bool retval = false;
	...
	if (dcol) {
		...
		dcol->unlockAccess("IdeccmbdAppsrv", "validateJrefNotify");
	};

	return retval;
};

uint IdeccmbdAppsrv::readDpchApp(
			XchgIdeccmbd* xchg
			, ReqIdec* req
		) {
	xmlDoc* doc = NULL;
...
void IdeccmbdAppsrv::writeDpchEng(
			XchgIdeccmbd* xchg
			, ReqIdec* req
		) {
	xmlBuffer* buf = NULL;
