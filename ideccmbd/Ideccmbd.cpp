/**
  * \file Ideccmbd.cpp
  * inter-thread exchange object for Idec combined daemon (implementation)
...
bool StgIdeccmbd::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIdeccmbd");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIdeccmbd";
...
void StgIdeccmbd::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIdeccmbd";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIdeccmbd";
...
set<uint> StgIdeccmbd::comm(
			const StgIdeccmbd* comp
		) {
	set<uint> items;
...
XchgdataIdecOpprc::XchgdataIdecOpprc() {
};
...
XchgIdeccmbd::XchgIdeccmbd() {
	...
	// scrambled ref codes
	Mutex::init(&mScr, true, "mScr", "XchgIdeccmbd", "XchgIdeccmbd");

	// log file
	Mutex::init(&mLogfile, true, "mLogfile", "XchgIdeccmbd", "XchgIdeccmbd");

	// condition for job processors
	Mutex::init(&mcJobprcs, true, "mcJobprcs", "XchgIdeccmbd", "XchgIdeccmbd");
	Cond::init(&cJobprcs, "cJobprcs", "XchgIdeccmbd", "XchgIdeccmbd");

	// condition for op processors
	Mutex::init(&mcOpprcs, true, "mcOpprcs", "XchgIdeccmbd", "XchgIdeccmbd");
	Cond::init(&cOpprcs, "cOpprcs", "XchgIdeccmbd", "XchgIdeccmbd");

	// request list
	Mutex::init(&mReqs, true, "mReqs", "XchgIdeccmbd", "XchgIdeccmbd");

	// operation invocation list
	orefseq = new Refseq("orefseq");
	Mutex::init(&mInvs, true, "mInvs", "XchgIdeccmbd", "XchgIdeccmbd");

	// presetting list
	Mutex::init(&mPresets, true, "mPresets", "XchgIdeccmbd", "XchgIdeccmbd");

	// stub manager list
	Mutex::init(&mStmgrs, true, "mStmgrs", "XchgIdeccmbd", "XchgIdeccmbd");

	// call listener list
	Mutex::init(&mClstns, true, "mClstns", "XchgIdeccmbd", "XchgIdeccmbd");

	// dispatch collector list
	Mutex::init(&mDcols, true, "mDcols", "XchgIdeccmbd", "XchgIdeccmbd");

	// job list
	jrefseq = new Refseq("jrefseq");
	Mutex::init(&mJobs, true, "mJobs", "XchgIdeccmbd", "XchgIdeccmbd");
...
XchgIdeccmbd::~XchgIdeccmbd() {
	...
	// scrambled ref codes
	Mutex::destroy(&mScr, true, "mScr", "XchgIdeccmbd", "~XchgIdeccmbd");

	// log file
	Mutex::destroy(&mLogfile, true, "mLogfile", "XchgIdeccmbd", "~XchgIdeccmbd");

	// condition for job processors
	Mutex::destroy(&mcJobprcs, true, "mcJobprcs", "XchgIdeccmbd", "~XchgIdeccmbd");
	Cond::destroy(&cJobprcs, "cJobprcs", "XchgIdeccmbd", "~XchgIdeccmbd");

	// condition for op processors
	Mutex::destroy(&mcOpprcs, true, "mcOpprcs", "XchgIdeccmbd", "~XchgIdeccmbd");
	Cond::destroy(&cOpprcs, "cOpprcs", "XchgIdeccmbd", "~XchgIdeccmbd");

	// request list
	Mutex::destroy(&mReqs, true, "mReqs", "XchgIdeccmbd", "~XchgIdeccmbd");

	// operation invocation list
	delete orefseq;
	Mutex::destroy(&mInvs, true, "mInvs", "XchgIdeccmbd", "~XchgIdeccmbd");

	// presetting list
	Mutex::destroy(&mPresets, true, "mPresets", "XchgIdeccmbd", "~XchgIdeccmbd");

	// stub manager list
	Mutex::destroy(&mStmgrs, true, "mStmgrs", "XchgIdeccmbd", "~XchgIdeccmbd");

	// call listener list
	Mutex::destroy(&mClstns, true, "mClstns", "XchgIdeccmbd", "~XchgIdeccmbd");

	// dispatch collector list
	Mutex::destroy(&mDcols, true, "mDcols", "XchgIdeccmbd", "~XchgIdeccmbd");

	// job list
	delete jrefseq;
	Mutex::destroy(&mJobs, true, "mJobs", "XchgIdeccmbd", "~XchgIdeccmbd");
...
void XchgIdeccmbd::startMon() {
	...
	Mutex::lock(&mJobs, "mJobs", "XchgIdeccmbd", "startMon");
	for (auto it=jobs.begin();it!=jobs.end();it++) {
		...
		if (dcol) {
			Dcol = true;
			dcol->unlockAccess("XchgIdeccmbd", "startMon");
		} else Dcol = false;

		stmgr = getStmgrByJref(job->jref);
		if (stmgr) {
			Stmgr = true;
			stmgr->unlockAccess("XchgIdeccmbd", "startMon");
		} else Stmgr = false;
	...
	Mutex::unlock(&mJobs, "mJobs", "XchgIdeccmbd", "startMon");

	Mutex::lock(&mClstns, "mClstns", "XchgIdeccmbd", "startMon");
	...
	Mutex::unlock(&mClstns, "mClstns", "XchgIdeccmbd", "startMon");

	Mutex::lock(&mPresets, "mPresets", "XchgIdeccmbd", "startMon");
	for (auto it=presets.begin();it!=presets.end();it++) {
		preset = it->second;
		mon.insertPreset(preset->pref.jref, VecIdecVPreset::getSref(preset->pref.ixVPreset), preset->arg.writeText());
	};
	Mutex::unlock(&mPresets, "mPresets", "XchgIdeccmbd", "startMon");

	triggerCall(NULL, VecIdecVCall::CALLIDECMONSTATCHG, jrefRoot);
};

void XchgIdeccmbd::stopMon() {
	mon.stop();

	triggerCall(NULL, VecIdecVCall::CALLIDECMONSTATCHG, jrefRoot);
};

void XchgIdeccmbd::appendToLogfile(
			const string& str
		) {
	time_t rawtime;
	fstream logfile;

	Mutex::lock(&mLogfile, "mLogfile", "XchgIdeccmbd", "appendToLogfile");
	...
	Mutex::unlock(&mLogfile, "mLogfile", "XchgIdeccmbd", "appendToLogfile");
};

void XchgIdeccmbd::addReq(
			ReqIdec* req
		) {
	Mutex::lock(&mReqs, "mReqs", "XchgIdeccmbd", "addReq");

	req->ixVState = ReqIdec::VecVState::WAITPRC;
	reqs.push_back(req);

	Mutex::unlock(&mReqs, "mReqs", "XchgIdeccmbd", "addReq");

	Cond::signal(&cJobprcs, &mcJobprcs, "cJobprcs", "mcJobprcs", "XchgIdeccmbd", "addReq");
};

ReqIdec* XchgIdeccmbd::pullFirstReq() {
	ReqIdec* req;

	// get first element from list
	Mutex::lock(&mReqs, "mReqs", "XchgIdeccmbd", "pullFirstReq");
	...
	Mutex::unlock(&mReqs, "mReqs", "XchgIdeccmbd", "pullFirstReq");

	return req;
};

void XchgIdeccmbd::addInvs(
			DbsIdec* dbsidec
			, const ubigint jref
	...
	if (job) {
		...
		Mutex::lock(&mInvs, "mInvs", "XchgIdeccmbd", "addInvs");
		Mutex::lock(&(job->mOps), "job->mOps", "XchgIdeccmbd", "addInvs");
		...
		Mutex::unlock(&(job->mOps), "job->mOps", "XchgIdeccmbd", "addInvs");
		Mutex::unlock(&mInvs, "mInvs", "XchgIdeccmbd", "addInvs");

		Cond::signal(&cOpprcs, &mcOpprcs, "cOpprcs", "mcOpprcs", "XchgIdeccmbd", "addInvs");
	};
};

DpchInvIdec* XchgIdeccmbd::pullFirstInv() {
	DpchInvIdec* inv;

	// get first element from list
	Mutex::lock(&mInvs, "mInvs", "XchgIdeccmbd", "pullFirstInv");
	...
	Mutex::unlock(&mInvs, "mInvs", "XchgIdeccmbd", "pullFirstInv");

	return inv;
};

Preset* XchgIdeccmbd::addPreset(
	...
	Mutex::lock(&mPresets, "mPresets", "XchgIdeccmbd", "addPreset");
	...
	Mutex::unlock(&mPresets, "mPresets", "XchgIdeccmbd", "addPreset");

	return(preset);
};

Preset* XchgIdeccmbd::addIxPreset(
			const uint ixIdecVPreset
			, const ubigint jref
			, const uint ix
		) {
...
Preset* XchgIdeccmbd::addRefPreset(
			const uint ixIdecVPreset
			, const ubigint jref
			, const ubigint ref
		) {
...
Preset* XchgIdeccmbd::addRefsPreset(
			const uint ixIdecVPreset
			, const ubigint jref
...
Preset* XchgIdeccmbd::addSrefPreset(
			const uint ixIdecVPreset
			, const ubigint jref
			, const string& sref
		) {
...
Preset* XchgIdeccmbd::addIntvalPreset(
			const uint ixIdecVPreset
			, const ubigint jref
			, const int intval
		) {
...
Preset* XchgIdeccmbd::addDblvalPreset(
			const uint ixIdecVPreset
			, const ubigint jref
			, const double dblval
		) {
...
Preset* XchgIdeccmbd::addBoolvalPreset(
			const uint ixIdecVPreset
			, const ubigint jref
			, const bool boolval
		) {
...
Preset* XchgIdeccmbd::addTxtvalPreset(
			const uint ixIdecVPreset
			, const ubigint jref
			, const string& txtval
		) {
...
Preset* XchgIdeccmbd::getPresetByPref(
			const presetref_t& pref
		) {
	Preset* preset = NULL;

	Mutex::lock(&mPresets, "mPresets", "XchgIdeccmbd", "getPresetByPref");

	auto it = presets.find(pref);
	if (it != presets.end()) preset = it->second;

	Mutex::unlock(&mPresets, "mPresets", "XchgIdeccmbd", "getPresetByPref");

	return preset;
};

Arg XchgIdeccmbd::getPreset(
			const uint ixIdecVPreset
			, const ubigint jref
		) {
	Arg arg;
	...
	if ( (ixIdecVPreset == VecIdecVPreset::PREIDECSYSDATE) || (ixIdecVPreset == VecIdecVPreset::PREIDECSYSTIME) || (ixIdecVPreset == VecIdecVPreset::PREIDECSYSSTAMP) ) {
		...
		Mutex::lock(&mJobs, "mJobs", "XchgIdeccmbd", "getPreset");
		Mutex::lock(&mPresets, "mPresets", "XchgIdeccmbd", "getPreset");
		...
		Mutex::unlock(&mPresets, "mPresets", "XchgIdeccmbd", "getPreset");
		Mutex::unlock(&mJobs, "mJobs", "XchgIdeccmbd", "getPreset");
	};

	return(arg);
};

uint XchgIdeccmbd::getIxPreset(
			const uint ixIdecVPreset
			, const ubigint jref
		) {
...
ubigint XchgIdeccmbd::getRefPreset(
			const uint ixIdecVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixIdecVPreset, jref);
...
vector<ubigint> XchgIdeccmbd::getRefsPreset(
			const uint ixIdecVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixIdecVPreset, jref);
...
string XchgIdeccmbd::getSrefPreset(
			const uint ixIdecVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixIdecVPreset, jref);
...
int XchgIdeccmbd::getIntvalPreset(
			const uint ixIdecVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixIdecVPreset, jref);
...
double XchgIdeccmbd::getDblvalPreset(
			const uint ixIdecVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixIdecVPreset, jref);
...
bool XchgIdeccmbd::getBoolvalPreset(
			const uint ixIdecVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixIdecVPreset, jref);
...
string XchgIdeccmbd::getTxtvalPreset(
			const uint ixIdecVPreset
			, const ubigint jref
		) {
	Arg arg = getPreset(ixIdecVPreset, jref);
...
void XchgIdeccmbd::getPresetsByJref(
			const ubigint jref
	...
	Mutex::lock(&mPresets, "mPresets", "XchgIdeccmbd", "getPresetsByJref");
	...
	Mutex::unlock(&mPresets, "mPresets", "XchgIdeccmbd", "getPresetsByJref");
};

void XchgIdeccmbd::removePreset(
			const uint ixIdecVPreset
			, const ubigint jref
		) {
	Mutex::lock(&mPresets, "mPresets", "XchgIdeccmbd", "removePreset");
	...
	Mutex::unlock(&mPresets, "mPresets", "XchgIdeccmbd", "removePreset");
};

void XchgIdeccmbd::removePresetsByJref(
			const ubigint jref
		) {
	Mutex::lock(&mPresets, "mPresets", "XchgIdeccmbd", "removePresetsByJref");
	...
	Mutex::unlock(&mPresets, "mPresets", "XchgIdeccmbd", "removePresetsByJref");
};

StmgrIdec* XchgIdeccmbd::addStmgr(
			const ubigint jref
			, const uint ixVNonetype
		) {
	StmgrIdec* stmgr = NULL;
	...
	Mutex::lock(&mStmgrs, "mStmgrs", "XchgIdeccmbd", "addStmgr");
	...
	Mutex::unlock(&mStmgrs, "mStmgrs", "XchgIdeccmbd", "addStmgr");

	return(stmgr);
};

StmgrIdec* XchgIdeccmbd::getStmgrByJref(
			const ubigint jref
		) {
	StmgrIdec* stmgr = NULL;

	Mutex::lock(&mStmgrs, "mStmgrs", "XchgIdeccmbd", "getStmgrByJref");

	auto it = stmgrs.find(jref);

	if (it != stmgrs.end()) {
		stmgr = it->second;
		stmgr->lockAccess("XchgIdeccmbd", "getStmgrByJref");
	};

	Mutex::unlock(&mStmgrs, "mStmgrs", "XchgIdeccmbd", "getStmgrByJref");

	return(stmgr);
};

void XchgIdeccmbd::removeStmgrByJref(
			const ubigint jref
		) {
	Mutex::lock(&mStmgrs, "mStmgrs", "XchgIdeccmbd", "removeStmgrByJref");
	...
	Mutex::unlock(&mStmgrs, "mStmgrs", "XchgIdeccmbd", "removeStmgrByJref");
};

Clstn* XchgIdeccmbd::addClstn(
	...
	Mutex::lock(&mClstns, "mClstns", "XchgIdeccmbd", "addClstn");
	...
	Mutex::unlock(&mClstns, "mClstns", "XchgIdeccmbd", "addClstn");

	return(clstn);
};

Clstn* XchgIdeccmbd::addIxClstn(
			const uint ixIdecVCall
			, const ubigint jref
			, const uint ixVJobmask
			, const ubigint jrefTrig
...
Clstn* XchgIdeccmbd::addRefClstn(
			const uint ixIdecVCall
			, const ubigint jref
			, const uint ixVJobmask
			, const ubigint jrefTrig
...
Clstn* XchgIdeccmbd::addIxRefClstn(
			const uint ixIdecVCall
			, const ubigint jref
			, const uint ixVJobmask
			, const ubigint jrefTrig
...
Clstn* XchgIdeccmbd::addIxRefSrefClstn(
			const uint ixIdecVCall
			, const ubigint jref
			, const uint ixVJobmask
			, const ubigint jrefTrig
...
Clstn* XchgIdeccmbd::addClstnStmgr(
			const uint ixIdecVCall
			, const ubigint jref
		) {
	...
	// create new clstn and append to clstn list
	Mutex::lock(&mClstns, "mClstns", "XchgIdeccmbd", "addClstnStmgr");
	...
	Mutex::unlock(&mClstns, "mClstns", "XchgIdeccmbd", "addClstnStmgr");

	return(clstn);
};

Clstn* XchgIdeccmbd::getClstnByCref(
			const clstnref_t& cref
		) {
	Clstn* clstn = NULL;

	Mutex::lock(&mClstns, "mClstns", "XchgIdeccmbd", "getClstnByCref");

	auto it = clstns.find(cref);
	if (it != clstns.end()) clstn = it->second;

	Mutex::unlock(&mClstns, "mClstns", "XchgIdeccmbd", "getClstnByCref");

	return clstn;
};

void XchgIdeccmbd::getClstnsByJref(
			const ubigint jref
			, const bool stmgr
	...
	Mutex::lock(&mClstns, "mClstns", "XchgIdeccmbd", "getClstnsByJref");
	...
	Mutex::unlock(&mClstns, "mClstns", "XchgIdeccmbd", "getClstnsByJref");
};

void XchgIdeccmbd::removeClstns(
			const uint ixIdecVCall
			, const ubigint jref
			, const bool stmgr
		) {
	Mutex::lock(&mClstns, "mClstns", "XchgIdeccmbd", "removeClstns");
	...
	Mutex::unlock(&mClstns, "mClstns", "XchgIdeccmbd", "removeClstns");
};

void XchgIdeccmbd::removeClstnsByJref(
			const ubigint jref
			, const bool stmgr
		) {
	Mutex::lock(&mClstns, "mClstns", "XchgIdeccmbd", "removeClstnsByJref");
	...
	Mutex::unlock(&mClstns, "mClstns", "XchgIdeccmbd", "removeClstnsByJref");
};

void XchgIdeccmbd::findJrefsByCall(
			Call* call
	...
	Mutex::lock(&mJobs, "mJobs", "XchgIdeccmbd", "findJrefsByCall");
	Mutex::lock(&mClstns, "mClstns", "XchgIdeccmbd", "findJrefsByCall");
	...
	Mutex::unlock(&mClstns, "mClstns", "XchgIdeccmbd", "findJrefsByCall");
	Mutex::unlock(&mJobs, "mJobs", "XchgIdeccmbd", "findJrefsByCall");
};

void XchgIdeccmbd::triggerCall(
			DbsIdec* dbsidec
			, Call* call
		) {
	vector<ubigint> jrefs;
	...
	for (unsigned int i=0;i<jrefs.size();i++) {
		...
		if (stmgrs[i]) {
			...
			if (stmgr) {
				...
				stmgr->unlockAccess("XchgIdeccmbd", "triggerCall");
			};
			...
			if (job) {
				if (ixVJactype != Clstn::VecVJactype::WEAK) {
					if (ixVJactype == Clstn::VecVJactype::LOCK) {
						job->lockAccess("XchgIdeccmbd", "triggerCall");

					} else if (ixVJactype == Clstn::VecVJactype::TRY) {
						if (Mutex::trylock(&(job->mAccess), "job(" + to_string(jref) + ")->mAccess", "XchgIdeccmbd", "triggerCall") == EBUSY) continue;
					};
				};
			...
			if (ixVJactype != Clstn::VecVJactype::WEAK) {
				job = getJobByJref(jref);
				if (job) job->unlockAccess("XchgIdeccmbd", "triggerCall");
			};
		};
...
bool XchgIdeccmbd::triggerArgToArgCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const Arg& argInv
...
bool XchgIdeccmbd::triggerIxCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgIdeccmbd::triggerRefCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
...
bool XchgIdeccmbd::triggerIntvalCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const int intvalInv
...
bool XchgIdeccmbd::triggerBoolvalCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
...
bool XchgIdeccmbd::triggerIxRefCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgIdeccmbd::triggerIxSrefCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgIdeccmbd::triggerIxIntvalCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgIdeccmbd::triggerIxDblvalCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgIdeccmbd::triggerIxTxtvalCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgIdeccmbd::triggerToBoolvalCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, bool& boolvalRet
...
bool XchgIdeccmbd::triggerIxToBoolvalCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgIdeccmbd::triggerRefToSrefCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const ubigint refInv
...
bool XchgIdeccmbd::triggerRefToBoolvalCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const ubigint refInv
...
bool XchgIdeccmbd::triggerIxRefToIxCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgIdeccmbd::triggerIxRefToRefCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgIdeccmbd::triggerIxRefSrefCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgIdeccmbd::triggerIxRefSrefIntvalCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgIdeccmbd::triggerIxRefSrefIntvalToRefCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const uint ixInv
...
bool XchgIdeccmbd::triggerRefToIxCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const ubigint refInv
...
bool XchgIdeccmbd::triggerSrefToRefCall(
			DbsIdec* dbsidec
			, const uint ixIdecVCall
			, const ubigint jref
			, const string& srefInv
...
DcolIdec* XchgIdeccmbd::addDcol(
	...
	Mutex::lock(&mJobs, "mJobs", "XchgIdeccmbd", "addDcol");
	Mutex::lock(&mDcols, "mDcols", "XchgIdeccmbd", "addDcol");
	...
	if (job) {
		...
		if (!dcol) {
			...
			dcol->lockAccess("XchgIdeccmbd", "addDcol");
		};
	...
	Mutex::unlock(&mDcols, "mDcols", "XchgIdeccmbd", "addDcol");
	Mutex::unlock(&mJobs, "mJobs", "XchgIdeccmbd", "addDcol");

	return(dcol);
};

DcolIdec* XchgIdeccmbd::getDcolByJref(
			const ubigint jref
			, const bool tree
		) {
	JobIdec* job = NULL;
	...
	Mutex::lock(&mJobs, "mJobs", "XchgIdeccmbd", "getDcolByJref");
	Mutex::lock(&mDcols, "mDcols", "XchgIdeccmbd", "getDcolByJref");
	...
	while (job != NULL) {
		...
		if (it != dcols.end()) {
			dcol = it->second;
			dcol->lockAccess("XchgIdeccmbd", "getDcolByJref");
...
void XchgIdeccmbd::removeDcolByJref(
			const ubigint jref
		) {
	Mutex::lock(&mDcols, "mDcols", "XchgIdeccmbd", "removeDcolByJref");
	...
	Mutex::unlock(&mDcols, "mDcols", "XchgIdeccmbd", "removeDcolByJref");
};

void XchgIdeccmbd::submitDpch(
			DpchEngIdec* dpcheng
		) {
	DcolIdec* dcol = NULL;
	...
	if (dcol) {
		...
		if (dcol->req != NULL) {
			...
			dcol->req = NULL;
			//cout << "XchgIdeccmbd::submitDpch() waiting request for ixIdecVDpch = " << dpcheng->ixIdecVDpch << ", jref = " << dpcheng->jref << endl;

			Cond::signal(cReady_backup, mcReady_backup, "dcol(" + to_string(dcol->jref) + ")->req->cReady", "dcol(" + to_string(dcol->jref) + ")->req->mcReady", "XchgIdeccmbd", "submitDpch");

			dcol->unlockAccess("XchgIdeccmbd", "submitDpch");
			...
			for (auto it=dcol->dpchs.begin();it!=dcol->dpchs.end();) {
				...
				if ((dpchtest->ixIdecVDpch == dpcheng->ixIdecVDpch) && (dpchtest->jref == dpcheng->jref)) {
					...
					//cout << "XchgIdeccmbd::submitDpch() merge for ixIdecVDpch = " << dpcheng->ixIdecVDpch << ", jref = " << dpcheng->jref << endl;
			...
			if (dpcheng) {
				...
				//cout << "XchgIdeccmbd::submitDpch() append for ixIdecVDpch = " << dpcheng->ixIdecVDpch << ", jref = " << dpcheng->jref << endl;
			};
...
DpchEngIdec* XchgIdeccmbd::pullFirstDpch(
			DcolIdec* dcol
		) {
...
ubigint XchgIdeccmbd::addJob(
			JobIdec* job
		) {
	JobIdec* supjob = NULL;

	// get new jref and append to job list
	Mutex::lock(&mJobs, "mJobs", "XchgIdeccmbd", "addJob");
	...
	Mutex::unlock(&mJobs, "mJobs", "XchgIdeccmbd", "addJob");
...
JobIdec* XchgIdeccmbd::getJobByJref(
			const ubigint jref
		) {
	JobIdec* job = NULL;

	Mutex::lock(&mJobs, "mJobs", "XchgIdeccmbd", "getJobByJref");

	auto it = jobs.find(jref);
	if (it != jobs.end()) job = it->second;

	Mutex::unlock(&mJobs, "mJobs", "XchgIdeccmbd", "getJobByJref");

	return job;
};

void XchgIdeccmbd::removeJobByJref(
			const ubigint jref
		) {
	JobIdec* supjob = NULL;
	JobIdec* job = NULL;
	...
	Mutex::lock(&mJobs, "mJobs", "XchgIdeccmbd", "removeJobByJref");
	...
	Mutex::unlock(&mJobs, "mJobs", "XchgIdeccmbd", "removeJobByJref");
};

void XchgIdeccmbd::transferMaster(
			DbsIdec* dbsidec
			, JobIdec* fromjob
			, JobIdec* tojob
		) {
...
void XchgIdeccmbd::offerMaster(
	...
	Mutex::lock(&mJobs, "mJobs", "XchgIdeccmbd", "offerMaster");
	...
	Mutex::unlock(&mJobs, "mJobs", "XchgIdeccmbd", "offerMaster");
};

bool XchgIdeccmbd::claimMaster(
			DbsIdec* dbsidec
			, const ubigint jref
		) {
	bool retval = false;
	...
	Mutex::lock(&mJobs, "mJobs", "XchgIdeccmbd", "claimMaster");
	...
	Mutex::unlock(&mJobs, "mJobs", "XchgIdeccmbd", "claimMaster");

	return retval;
};

ubigint XchgIdeccmbd::addWakeup(
			const ubigint jref
			, const string sref
			, const unsigned int deltat
			, const bool weak
	...
	if (deltat == 0) {
		...
		if (res != 0) cout << "XchgIdeccmbd::addWakeup() error creating timer thread (" << res << ")" << endl;
		res = pthread_detach(timer);
		if (res != 0) cout << "XchgIdeccmbd::addWakeup() error detaching timer thread (" << res << ")" << endl;
	};

	return(wref);
};

void* XchgIdeccmbd::runWakeup(
			void* arg
		) {
