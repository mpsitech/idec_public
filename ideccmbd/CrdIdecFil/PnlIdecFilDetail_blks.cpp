/**
  * \file PnlIdecFilDetail_blks.cpp
  * job handler for job PnlIdecFilDetail (implementation of blocks)
...
void PnlIdecFilDetail::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfIdecFilDetail";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfIdecFilDetail";
...
void PnlIdecFilDetail::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppIdecFilDetail";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppIdecFilDetail";
...
void PnlIdecFilDetail::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrIdecFilDetail";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrIdecFilDetail";
...
void PnlIdecFilDetail::Tag::writeXML(
			const uint ixIdecVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagIdecFilDetail";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemIdecFilDetail";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		...
		writeStringAttr(wr, itemtag, "sref", "CptClu", VecIdecVTag::getTitle(VecIdecVTag::CLUST, ixIdecVLocale));
	xmlTextWriterEndElement(wr);
};
...
PnlIdecFilDetail::DpchAppData::DpchAppData() : DpchAppIdec(VecIdecVDpch::DPCHAPPIDECFILDETAILDATA) {
};
...
void PnlIdecFilDetail::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppIdecFilDetailData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlIdecFilDetail::DpchAppDo::DpchAppDo() : DpchAppIdec(VecIdecVDpch::DPCHAPPIDECFILDETAILDO) {
	ixVDo = 0;
};
...
void PnlIdecFilDetail::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppIdecFilDetailDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlIdecFilDetail::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFLstClu
			...
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngIdec(VecIdecVDpch::DPCHENGIDECFILDETAILDATA, jref) {
...
void PnlIdecFilDetail::DpchEngData::merge(
			DpchEngIdec* dpcheng
		) {
...
void PnlIdecFilDetail::DpchEngData::writeXML(
			const uint ixIdecVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngIdecFilDetailData");
