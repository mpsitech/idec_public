/**
  * \file JobIdecRecord_blks.cpp
  * job handler for job JobIdecRecord (implementation of blocks)
...
/******************************************************************************
 class JobIdecRecord::VecVSge
 ******************************************************************************/

uint JobIdecRecord::VecVSge::getIx(
			const string& sref
		) {
...
string JobIdecRecord::VecVSge::getSref(
			const uint ix
		) {
	...
	else if (ix == PAUSE) return("pause");

	return("");
};

void JobIdecRecord::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
