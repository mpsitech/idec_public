/**
  * \file JobIdecFusion_blks.cpp
  * job handler for job JobIdecFusion (implementation of blocks)
...
/******************************************************************************
 class JobIdecFusion::VecVSge
 ******************************************************************************/

uint JobIdecFusion::VecVSge::getIx(
			const string& sref
		) {
...
string JobIdecFusion::VecVSge::getSref(
			const uint ix
		) {
...
void JobIdecFusion::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
