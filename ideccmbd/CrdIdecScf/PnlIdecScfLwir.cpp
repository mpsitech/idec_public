/**
  ...
  * job handler for job PnlIdecScfLwir (implementation)
...
#include "PnlIdecScfLwir_blks.cpp"
#include "PnlIdecScfLwir_evals.cpp"

/******************************************************************************
 class PnlIdecScfLwir
 ******************************************************************************/

PnlIdecScfLwir::PnlIdecScfLwir(
			XchgIdec* xchg
			, DbsIdec* dbsidec
			, const ubigint jrefSup
			, const uint ixIdecVLocale
		) : JobIdec(xchg, VecIdecVJob::PNLIDECSCFLWIR, jrefSup, ixIdecVLocale) {
...
// IP cust --- INSERT
DpchEngIdec* PnlIdecScfLwir::getNewDpchEng(
			set<uint> items
		) {
...
void PnlIdecScfLwir::refresh(
	...
	contiac.TxfLm0 = to_string(xchg->stgjobideclwiracq.lam0);
	contiac.TxfLm1 = to_string(xchg->stgjobideclwiracq.lam1);
	...
	if (joblwiracq) continf.ButMasterOn = (joblwiracq->jref == joblwiracq->jrefMast);

	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);

	// statshr
	statshr.TxfLm0Active = evalTxfLm0Active(dbsidec);
	statshr.TxfLm1Active = evalTxfLm1Active(dbsidec);
	// IP refresh --- REND

	if (statshr.diff(&oldStatshr).size() != 0) insert(moditems, DpchEngData::STATSHR);
};

void PnlIdecScfLwir::handleRequest(
			DbsIdec* dbsidec
			, ReqIdec* req
		) {
	if (req->ixVBasetype == ReqIdec::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixIdecVDpch == VecIdecVDpch::DPCHAPPIDECSCFLWIRDATA) {
		...
		} else if (req->dpchapp->ixIdecVDpch == VecIdecVDpch::DPCHAPPIDECSCFLWIRDO) {
...
void PnlIdecScfLwir::handleDpchAppIdecInit(
			DbsIdec* dbsidec
...
void PnlIdecScfLwir::handleDpchAppDataContiac(
			DbsIdec* dbsidec
			, ContIac* _contiac
			, DpchEngIdec** dpcheng
		) {
	...
	if (has(diffitems, ContIac::TXFLM0)) contiac.TxfLm0 = _contiac->TxfLm0;
	if (has(diffitems, ContIac::TXFLM1)) contiac.TxfLm1 = _contiac->TxfLm1;
...
void PnlIdecScfLwir::handleDpchAppDoButMasterClick(
			DbsIdec* dbsidec
			, DpchEngIdec** dpcheng
		) {
...
void PnlIdecScfLwir::handleCall(
			DbsIdec* dbsidec
			, Call* call
		) {
...
bool PnlIdecScfLwir::handleCallIdecStgChg(
			DbsIdec* dbsidec
...
bool PnlIdecScfLwir::handleCallIdecMastslvChg(
			DbsIdec* dbsidec
			, const ubigint jrefTrig
		) {
	bool retval = false;
