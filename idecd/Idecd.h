/**
  * \file Idecd.h
  * inter-thread exchange object for Idec daemon (declarations)
...
class XchgIdecd;
typedef XchgIdecd XchgIdec;

/**
  * DpchIdecdAck (written by dOpengconsrv, identified by opd_exe)
  */
...
/**
  * DpchIdecdQuit (written by d_exe, identified by opdEngsrv)
  */
...
/**
  * DpchIdecdReg (written by dOpengconsrv, read by opd_exe)
  */
...
/**
  * DpchIdecopdReg (written by opd_exe, read by dOpengconsrv)
  */
class DpchIdecopdReg : public DpchIdec {

public:
	static const uint PORT = 1;
	static const uint IXIDECVOPENGTYPE = 2;
...
/**
  * DpchIdecopdUnreg (written by opd_exe, read by dOpengconsrv)
  */
class DpchIdecopdUnreg : public DpchIdec {
...
public:
	StgIdecd(const usmallint jobprcn = 1, const usmallint opengsrvport = 0, const usmallint appsrvport = 0, const bool https = false, const usmallint histlength = 20);
...
public:
	...
	set<uint> comm(const StgIdecd* comp);
	set<uint> diff(const StgIdecd* comp);
};
...
public:
	ReqopengconIdec(const uint ixVState = VecVState::RECEIVE, const string& ip = "");
	~ReqopengconIdec();
...
public:
	ConopengIdec(const string& ip = "", const uint port = 0);
	~ConopengIdec();
...
public:
	NodeIdec(const ubigint nref, const string& ip, const uint port, const uint opprcn, const uint ixIdecVOpengtype);
	~NodeIdec();
...
public:
	...
	bool pullFirstInv(DpchInvIdec** dpchinv, NodeIdec** node);
	void returnInv(DpchInvIdec* dpchinv);
	...
	// node methods
	NodeIdec* addNode(const string& ip, const uint port, const uint opprcn, const uint ixIdecVOpengtype);
	NodeIdec* getNodeByNref(const ubigint nref);
