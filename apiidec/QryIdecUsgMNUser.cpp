/**
  ...
  * API code for job QryIdecUsgMNUser (implementation)
...
QryIdecUsgMNUser::StatApp::StatApp(
			const uint firstcol
...
bool QryIdecUsgMNUser::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryIdecUsgMNUser");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryIdecUsgMNUser";
...
set<uint> QryIdecUsgMNUser::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryIdecUsgMNUser::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryIdecUsgMNUser::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryIdecUsgMNUser");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryIdecUsgMNUser";
