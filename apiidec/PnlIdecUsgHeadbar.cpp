/**
  ...
  * API code for job PnlIdecUsgHeadbar (implementation)
...
PnlIdecUsgHeadbar::StgInf::StgInf(
...
bool PnlIdecUsgHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfIdecUsgHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfIdecUsgHeadbar";
...
set<uint> PnlIdecUsgHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlIdecUsgHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlIdecUsgHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagIdecUsgHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemIdecUsgHeadbar";
...
PnlIdecUsgHeadbar::DpchEngData::DpchEngData() : DpchEngIdec(VecIdecVDpch::DPCHENGIDECUSGHEADBARDATA) {
};
...
void PnlIdecUsgHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngIdecUsgHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
