/**
  ...
  * API code for job QryIdecUsgAAccess (implementation)
...
QryIdecUsgAAccess::StatApp::StatApp(
			const uint firstcol
...
bool QryIdecUsgAAccess::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryIdecUsgAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryIdecUsgAAccess";
...
set<uint> QryIdecUsgAAccess::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryIdecUsgAAccess::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryIdecUsgAAccess::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryIdecUsgAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryIdecUsgAAccess";
