/**
  ...
  * API code for job QryIdecUsgList (implementation)
...
QryIdecUsgList::StatApp::StatApp(
			const uint firstcol
...
bool QryIdecUsgList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryIdecUsgList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryIdecUsgList";
...
set<uint> QryIdecUsgList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryIdecUsgList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryIdecUsgList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryIdecUsgList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryIdecUsgList";
