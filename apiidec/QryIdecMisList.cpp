/**
  ...
  * API code for job QryIdecMisList (implementation)
...
/******************************************************************************
 class QryIdecMisList::VecVOrd
 ******************************************************************************/

uint QryIdecMisList::VecVOrd::getIx(
			const string& sref
		) {
	...
	else if (s == "sta") return STA;

	return(0);
};

string QryIdecMisList::VecVOrd::getSref(
			const uint ix
		) {
	if (ix == TIT) return("tit");
	else if (ix == GRP) return("grp");
	else if (ix == OWN) return("own");
	else if (ix == STA) return("sta");

	return("");
};

/******************************************************************************
 class QryIdecMisList::StatApp
 ******************************************************************************/

QryIdecMisList::StatApp::StatApp(
			const uint firstcol
...
bool QryIdecMisList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryIdecMisList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryIdecMisList";
...
set<uint> QryIdecMisList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryIdecMisList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryIdecMisList::StatShr
 ******************************************************************************/

QryIdecMisList::StatShr::StatShr(
			const uint ntot
...
bool QryIdecMisList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryIdecMisList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryIdecMisList";
...
set<uint> QryIdecMisList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryIdecMisList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryIdecMisList::StgIac
 ******************************************************************************/

QryIdecMisList::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryIdecMisList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryIdecMisList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryIdecMisList";
...
void QryIdecMisList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryIdecMisList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryIdecMisList";
...
set<uint> QryIdecMisList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryIdecMisList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
