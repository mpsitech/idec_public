/**
  ...
  * API code for job DlgIdecFilExtrmed (implementation)
...
bool DlgIdecFilExtrmed::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfDlgIdecFilExtrmed");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfDlgIdecFilExtrmed";
...
DlgIdecFilExtrmed::StatApp::StatApp(
			const bool initdone
...
bool DlgIdecFilExtrmed::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppDlgIdecFilExtrmed");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppDlgIdecFilExtrmed";
...
set<uint> DlgIdecFilExtrmed::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> DlgIdecFilExtrmed::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool DlgIdecFilExtrmed::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagDlgIdecFilExtrmed");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemDlgIdecFilExtrmed";
...
DlgIdecFilExtrmed::DpchAppData::DpchAppData(
			const string& scrJref
			, ContIac* contiac
			, const set<uint>& mask
		) : DpchAppIdec(VecIdecVDpch::DPCHAPPDLGIDECFILEXTRMEDDATA, scrJref) {
...
void DlgIdecFilExtrmed::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppDlgIdecFilExtrmedData");
...
DlgIdecFilExtrmed::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppIdec(VecIdecVDpch::DPCHAPPDLGIDECFILEXTRMEDDO, scrJref) {
...
void DlgIdecFilExtrmed::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppDlgIdecFilExtrmedDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/idec");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
DlgIdecFilExtrmed::DpchEngData::DpchEngData() : DpchEngIdec(VecIdecVDpch::DPCHENGDLGIDECFILEXTRMEDDATA) {
...
void DlgIdecFilExtrmed::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngDlgIdecFilExtrmedData");
	else
		basefound = checkXPath(docctx, basexpath);
