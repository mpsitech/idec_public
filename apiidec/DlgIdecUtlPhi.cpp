/**
  ...
  * API code for job DlgIdecUtlPhi (implementation)
...
bool DlgIdecUtlPhi::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfDlgIdecUtlPhi");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfDlgIdecUtlPhi";
...
bool DlgIdecUtlPhi::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppDlgIdecUtlPhi");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppDlgIdecUtlPhi";
...
set<uint> DlgIdecUtlPhi::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> DlgIdecUtlPhi::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool DlgIdecUtlPhi::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagDlgIdecUtlPhi");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemDlgIdecUtlPhi";
...
DlgIdecUtlPhi::DpchAppData::DpchAppData(
			const string& scrJref
			, ContIac* contiac
			, const set<uint>& mask
		) : DpchAppIdec(VecIdecVDpch::DPCHAPPDLGIDECUTLPHIDATA, scrJref) {
...
void DlgIdecUtlPhi::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppDlgIdecUtlPhiData");
...
DlgIdecUtlPhi::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppIdec(VecIdecVDpch::DPCHAPPDLGIDECUTLPHIDO, scrJref) {
...
void DlgIdecUtlPhi::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppDlgIdecUtlPhiDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/idec");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
DlgIdecUtlPhi::DpchEngData::DpchEngData() : DpchEngIdec(VecIdecVDpch::DPCHENGDLGIDECUTLPHIDATA) {
...
void DlgIdecUtlPhi::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngDlgIdecUtlPhiData");
	else
		basefound = checkXPath(docctx, basexpath);
