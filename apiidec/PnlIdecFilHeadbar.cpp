/**
  ...
  * API code for job PnlIdecFilHeadbar (implementation)
...
PnlIdecFilHeadbar::StgInf::StgInf(
...
bool PnlIdecFilHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfIdecFilHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfIdecFilHeadbar";
...
set<uint> PnlIdecFilHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlIdecFilHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlIdecFilHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagIdecFilHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemIdecFilHeadbar";
...
PnlIdecFilHeadbar::DpchEngData::DpchEngData() : DpchEngIdec(VecIdecVDpch::DPCHENGIDECFILHEADBARDATA) {
};
...
void PnlIdecFilHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngIdecFilHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
