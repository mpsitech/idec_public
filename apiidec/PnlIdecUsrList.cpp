/**
  ...
  * API code for job PnlIdecUsrList (implementation)
...
bool PnlIdecUsrList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfIdecUsrList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfIdecUsrList";
...
bool PnlIdecUsrList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrIdecUsrList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrIdecUsrList";
...
bool PnlIdecUsrList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagIdecUsrList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemIdecUsrList";
...
PnlIdecUsrList::DpchAppData::DpchAppData(
			const string& scrJref
			, ContIac* contiac
			, StgIac* stgiac
			, QryIdecUsrList::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchAppIdec(VecIdecVDpch::DPCHAPPIDECUSRLISTDATA, scrJref) {
...
void PnlIdecUsrList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppIdecUsrListData");
...
PnlIdecUsrList::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppIdec(VecIdecVDpch::DPCHAPPIDECUSRLISTDO, scrJref) {
...
void PnlIdecUsrList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppIdecUsrListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/idec");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlIdecUsrList::DpchEngData::DpchEngData() : DpchEngIdec(VecIdecVDpch::DPCHENGIDECUSRLISTDATA) {
...
void PnlIdecUsrList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngIdecUsrListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryIdecUsrList::StatApp();
		statshrqry = QryIdecUsrList::StatShr();
